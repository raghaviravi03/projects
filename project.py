...
df = df.assign(bins = pd.cut(df["driver_age"], [0,20,40,60,80,100], 
                        labels=['0 to 16', '16 to 40', '40 to 60', '60 to 80', '80 to 100']))
df.rename(columns={'bins':'driver_age_group'}, inplace=True)
a=[i.split(',')[0] for i in df.search_type]
df['search_type_agg']=a

st.title("Open Policing Dataset Explorer") # App Title
with st.sidebar:
    selected = option_menu("Menu", ["Homepage","Exploratory Analysis","Demographic Analysis","Predictions"],
                          default_index=0,
...
if selected == "Homepage":
    ...
    "Data Summary", "Sample Data Explorer", "Educational Resources"],
    ...
    if selected_home == "About the dataset":
    ...
    elif selected_home == "Data Summary":
        st.markdown("1. **Total Number of Records:** Overall, there are 91741 stops recorded in the dataset.")
        st.markdown("2. **Unique Categories of Stops:** The reason behind the stops include *Speeding*, *Moving Violation*, *Equipment*, *Registration*, and *Seat Belt*")
        st.markdown("3. **Demographic Information:**")
        st.markdown("- *Race* includes values such as White, Black, Hispanic, Asian, and other.")
        st.markdown("- *Gender* which includes values such as Male and Female.")
        st.markdown("- *Age* ranging from 15 to 99.")
        st.markdown('''<style>[data-testid="stMarkdownContainer"] ul{list-style-position: inside;}</style>''', unsafe_allow_html=True)
        st.markdown("4. **Reason for Stops:**")
        ...
        st.markdown('''<style>[data-testid="stMarkdownContainer"] ul{list-style-position: inside;}</style>''', unsafe_allow_html=True)
        st.markdown("5. **Outcome of Stops:**")
        ...
        st.dataframe(df[selected_de][df.year==choose_year].head())        
    
    elif selected_home == "Educational Resources":
        st.write("Kindly visit the below link if you are interested to learn more about this dataset,")
        ...
        st.write("[Simple Rules to guide expert classifications](%s)" % url2)
        ...
        url4 = "https://www.youtube.com/watch?v=f26QJKREYB8&t=8s"
        ...
        url5 = "https://bja.ojp.gov/sites/g/files/xyckuh186/files/Publications/NSA_NW_Manual.pdf"
    ...
    selected_exp = option_menu(None, ["Violation vs Other Attributes", "Drug Usage", "Time Series Trends"], 
    default_index=0, orientation="horizontal")
    ...
    if selected_exp == "Violation vs Other Attributes":
       ...
       #.set_xlabel("Hour of the Day")
        #ax.set_ylabel("Number of Drug Related Stops")
        st.pyplot()
        ...
        df.loc[df.sort_values(by="driver_age").drugs_related_stop, 'driver_age'].value_counts().sort_index().plot(figsize=(12, 6))
        st.pyplot()
        ...
        df.loc[df.sort_values(by="driver_race").drugs_related_stop, 'driver_race'].value_counts().sort_index().plot(figsize=(12, 6))
        st.pyplot()
    ...
    if selected_dem == "Searches vs Stops":
        ...
        temp = 100*df.groupby(by=['year',choose_hue])['search_conducted'].sum()/(df.groupby(by=['year',choose_hue])['search_conducted'].count())
        ...
        st.markdown("1. We observe a notable trend indicating that individuals within the age range of 80 to 100 years are consistently exempt from searches.")
        st.markdown("2. Additionally, it is reasonable to infer that the search percentage diminishes progressively as individuals transition from lower age brackets to higher age brackets.")
        st.markdown("3. Black and Hispanic individuals undergo more frequent searches, whereas Asians and Whites experience fewer searches. Nevertheless, there are observable trends indicating that the search rates for Black and Hispanic individuals are gradually decreasing and approaching those of Asians and Whites.")
        st.markdown("The observed trend where individuals in the age groups 80 to 100 are never searched, and the decreasing search percentage as age brackets increase, can be influenced by several factors such as,")
        ...
        temp = 100*df.groupby(by=['year',choose_hue])['is_arrested'].sum()/(df.groupby(by=['year',choose_hue])['is_arrested'].count())
        temp = temp.reset_index().set_index('year')
        temp = temp[(temp.index>=min_max_year[0]) & (temp.index<=min_max_year[1])]
        fig = px.line(temp,color=choose_hue)
        fig.update_xaxes(showgrid=False, linecolor = "#BCCCDC")
        ...
        st.subheader("Let's uncover the story behind the graph!")
        st.markdown("1. The arrest rates for Black and Hispanic individuals significantly decreased in 2007, nearly aligning with the rates for Whites and Asians, a trend that has remained consistent since then.")
        st.markdown("2. Arrest rates for men surpass those of women, although the gender gap is gradually narrowing, with men's arrest rates approaching those of women.")
        st.markdown("3. Individuals between the ages of 16 and 40 exhibit higher arrest rates compared to any other age group, while those in the age group of 60 to 100 are rarely subject to arrest.")
        
        st.markdown("The reduction in arrest rates for Black and Hispanic individuals could be influenced by a range of factors like,")
        ...
        st.markdown("The gradual narrowing of the arrest rate gap between men and women could be influenced by several factors:")
        st.markdown("- **Changing Social Norms:** Evolving societal attitudes might lead to equal recognition of criminal behavior regardless of gender, resulting in more equitable arrest rates.")
       
        st.markdown("The variation in arrest rates across age groups can be attributed to a range of social, economic, and behavioral factors such as,")
        st.markdown("- **Youthful Indiscretions:** Individuals in the 16 to 40 age range might be more prone to impulsive behavior and risk-taking, leading to higher engagement in activities that could result in arrest.")
        st.markdown("- **Socioeconomic Factors:** Economic instability and lack of opportunities can lead to involvement in criminal activities, especially among younger adults who may resort to illegal means for financial stability.")
        st.markdown("- **Employment Opportunities:** Limited job prospects for younger individuals can lead to frustration and involvement in illegal activities, contributing to higher arrest rates.")
        
        st.markdown('''<style>[data-testid="stMarkdownContainer"] ul{list-style-position: inside;}</style>''', unsafe_allow_html=True)
        
        
    elif selected_dem == "Stop Duration across years":
        st.subheader("Trends in stop duration across years")
        ...
        st.subheader("Let's uncover the story behind the graph!")
        st.markdown("1. For individuals within the age group of 0 to 16, the duration of police stops is higher in comparison to other age groups.")
        st.markdown("2. For elderly individuals, specifically those aged between 80 and 100, the duration of police stops shows fluctuations.")
        st.markdown("3. The duration of police stops is longer for women in comparison to men.")
        
        st.markdown("The longer stop durations for individuals aged 0 to 16 could be influenced by several factors:")
        ...
        st.markdown("The fluctuations in stop duration for individuals aged 80 to 100 could be influenced by various factors such as,")
        ...
        for i in df[choose_column].unique():
            ...
            across_ethnicities = dataframe.groupby('driver_age')['Speeding_violation'].sum()/dataframe.groupby('driver_age')['Speeding_violation'].count()
        ...
        fig=px.scatter(temp,x='Speeding_violation_x',y='Speeding_violation_y',size='age',width=400,height=400, 
                       labels={'Speeding_violation_x':column1, 'Speeding_violation_y':column2})
        fig.update_yaxes(range=[-0.1, 1.2])
        fig.update_xaxes(range=[-0.1, 1.2])
